...
import {
  ...
  mapItemSchemaToCollectionSchema,
} from "src/lib/schema/validate";
...
jest.mock("src/bin/firestore-schema/validate/params", () => {
  const originalModule = jest.requireActual(
    "src/bin/firestore-schema/validate/params"
  );
  return {
    ...originalModule,
...
describe("Validate Command", () => {
  ...
  beforeEach(() => {
    ...
    (firestoreUtils.getCredentialsFromFile as jest.Mock).mockResolvedValue(
      mockCredentials
    );
    (firestoreUtils.initFirestore as jest.Mock).mockResolvedValue(
      mockFirestore
    );
  ...
  test("fetches credentials from specified path", async () => {
    await executeAsyncValidateCommand(mockProgram);
    expect(firestoreUtils.getCredentialsFromFile).toHaveBeenCalledWith(
  ...
  describe("console output", () => {
    ...
    test("logs error message when validation fails", async () => {
      const mockErrors: ErrorObject[] = [
        {
          instancePath: "/0/name",
          keyword: "type",
